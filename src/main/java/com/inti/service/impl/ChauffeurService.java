package com.inti.service.impl;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.inti.entities.Chauffeur;
import com.inti.repositories.ChauffeurRepository;
import com.inti.service.interfaces.IChauffeurService;

@Service
public class ChauffeurService implements IChauffeurService{
	@Autowired
	ChauffeurRepository chauffeurRepository;

	@Override
	public List<Chauffeur> findAll() {
		return chauffeurRepository.findAll();
	}

	@Override
	public Chauffeur findOne(Long id) {
		return chauffeurRepository.findById(id).get();
	}

	@Override
	public Chauffeur save(Chauffeur chauffeur) {
		return chauffeurRepository.save(chauffeur);
	}

	@Override
	public void delete(Long id) {
		chauffeurRepository.deleteById(id);
	}

	@Override
	public List<Chauffeur> findByNomChauffeur(String nomChauffeur) {
		return chauffeurRepository.findByNomChauffeur(nomChauffeur);
	}

	@Override
	public List<Chauffeur> findByPrenomChauffeur(String prenomChauffeur) {
		return chauffeurRepository.findByPrenomChauffeur(prenomChauffeur);
	}

}
